...
class pix_attention(Module):
    ...
    def forward(self,args,index,index_len,x,x_teature):
        with torch.no_grad():
            gpu = 0
            ...
            attention_s = np.zeros((index_len,1,65536)).astype(np.float32)
            attention_s = torch.from_numpy(attention_s).cuda(gpu)
            attention_t = np.zeros((index_len,1,65536)).astype(np.float32)
            attention_t = torch.from_numpy(attention_t).cuda(gpu)
            ...
            for i in range(index_len):
                ...
                proj_query  = select_pix_T.permute(0,2,1).to(0)
                proj_key    = x_key_T[num[0]].view(1,5, width*height).to(0)      
                energy_T      = torch.bmm(proj_query,proj_key)
                attention_T = self.softmax(energy_T)

                loss = F.mse_loss(attention_S,attention_T)
                losssum = loss + losssum
            print("losssum:%f "%losssum)
            print("index_len:%f"%index_len)

        return losssum

    
def random_pix(output_student,output_teature,target):
    pixnum = 3000
    m_batchsize, C, height, width = output_student.size()


    x = range(0,256*256*m_batchsize,1)
    y = random.sample(x, pixnum*m_batchsize)#batch×3000の乱数を得る
    #print(y)
    list_index = []
    for i in range (pixnum*m_batchsize):
        index_batch = y[i] // 65536
        amari = y[i] % 65536
        index_q = amari // 256
        if index_q > 255:
            index_q = index_q -256
        ...
        index_mod = amari % 256
        list_index.append((index_batch, index_q, index_mod))
        #print(list_index)
